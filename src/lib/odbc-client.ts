...
const connStr =
  ...
  `KeyStorePrincipalId=${process.env.AKV_CLIENT_ID};` +
  `KeyStoreSecret=${process.env.AKV_CLIENT_SECRET};`;
...
export async function insertTeamMemberAE(data: {
  ...
  // 1) Duplicate email
  {
    const rows = await cn.query<{ id: string }>(
      ...
      [data.email] as (string | number | null)[]
    );
    if (rows.length > 0) {
  ...
  if (data.teamTricode && (role === 'COACH' || role === 'LEADER')) {
    const rows = await cn.query<{ id: string }>(
      ...
      [data.teamTricode, role] as (string | number | null)[]
    );
    if (rows.length > 0) {
  ...
  const result = await cn.query<{ id: string }>(sql, params as (string | number | null)[]);
  return result[0]?.id;
}

/* ------------------------ UPDATE (enforced) ------------------------ */
export async function updateTeamMemberAE(id: string, patch: {
  ...
  // Read current (for rule checks on role/email/tricode changes)
  const cur = await cn.query<{
    email: string; teamTricode: string | null; role: string;
  }>(
  ...
  // A) If email changed -> enforce duplicate
  if (nextEmail.toLowerCase() !== cur[0].email.toLowerCase()) {
    const dup = await cn.query<{ id: string }>(
      ...
      [nextEmail, id] as (string | number | null)[]
    );
    if (dup.length > 0) {
      const err: SqlError = new Error('Duplicate email');
      err.number = 2627;
  ...
  // B) If role/tricode imply leader/coach -> enforce one per team
  if (nextTri && (nextRole === 'COACH' || nextRole === 'LEADER')) {
    const clash = await cn.query<{ id: string }>(
      ...
      [nextTri, nextRole, id] as (string | number | null)[]
    );
    if (clash.length > 0) {
  ...
  const push = (col: string, v: unknown, transform?: (x: any) => any) => {
    const val = (transform ? transform(v) : v) as string | null | undefined;
    if (val !== undefined) { sets.push(`${col} = ?`); vals.push(nn(val)); }
  };
  ...
  const result = await cn.query<{ id: string }>(sql, vals as (string | number | null)[]);
  return result[0]?.id;
}
...
export async function readLatestTeamMembers(limit = 5) {
  ...
  const rows = await cn.query<{
    ...
    teamName: string | null; teamTricode: string | null; role: string; phone: string | null;
  }>(`
    SELECT TOP (${limit})
