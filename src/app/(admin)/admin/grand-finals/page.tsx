...
export default function GrandFinalsAdmin() {
  ...
  const load = async () => {
    try {
      const r = await fetch("/api/grand-finals", { cache: "no-store" });
  ...
  async function onDelete(id: string) {
    ...
    const r = await fetch(`/api/grand-finals/${id}`, { method: "DELETE" });
    if (!r.ok) {
  ...
  return (
    <div className="container mx-auto p-6 space-y-8">
      ...
      <form onSubmit={onSubmit} className="space-y-4 border p-4 rounded-xl">
        <div className="grid gap-3">
          ...
          <input
            name="title"
            required
            placeholder="Admin label e.g. Seoul GF Banner"
      ...
      <div className="grid md:grid-cols-3 gap-6">
        {items.map((it, idx) => {
          const src = `/api/grand-finals/${it.id}/image?t=${encodeURIComponent(it.updatedAt)}`;
          const image = (
            <Image
              src={src}
              alt={it.title}
